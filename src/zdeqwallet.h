...
#ifndef Dequant_ZDEQWALLET_H
#define Dequant_ZDEQWALLET_H
...
public:
    CzDEQWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZDEQ(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZDEQ(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Dequant_ZDEQWALLET_H
