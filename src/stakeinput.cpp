...
// The zDEQ block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zDEQ is a hash of the serial
...
{
    //Create an output returning the zDEQ that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZDEQOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zDEQ output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zDEQ", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZDEQOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zDEQ output", __func__);
...
{
    CzDEQTracker* zdeqTracker = pwallet->zdeqTracker.get();
    CMintMeta meta;
    if (!zdeqTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zdeqTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a DEQ stake is the outpoint
