...
CzDEQWallet::CzDEQWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zdeq seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZDEQSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZDEQSeed_deprecated()) {
                    LogPrintf("%s: Updated zDEQ seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zdeq seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zdeq wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzDEQWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZDEQCount(nCountLastUsed);
    else if (!walletdb.ReadZDEQCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDEQWallet::Lock()
{
    seedMaster = 0;
}

void CzDEQWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDEQWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZDEQ(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDEQWallet::LoadMintPoolFromDB()
{
...
void CzDEQWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDEQWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDEQWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zdeqTracker->HasPubcoinHash(pMint.first)) {
...
bool CzDEQWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZDEQ(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zdeqTracker which also adds to database
    pwalletMain->zdeqTracker->Add(dMint, true);
...
void CzDEQWallet::SeedToZDEQ(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDEQWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDEQWallet::GenerateDeterministicZDEQ(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDEQWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDEQWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
