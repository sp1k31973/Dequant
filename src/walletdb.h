...
public:
    ...
    bool WriteZDEQSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZDEQSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZDEQSeed_deprecated(uint256& seed);
    bool EraseZDEQSeed();
    bool EraseZDEQSeed_deprecated();

    bool WriteZDEQCount(const uint32_t& nCount);
    bool ReadZDEQCount(uint32_t& nCount);
