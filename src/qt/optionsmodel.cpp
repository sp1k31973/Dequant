...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::DEQ);
    ...
    if (!settings.contains("nAnonymizeDequantAmount"))
        settings.setValue("nAnonymizeDequantAmount", 1000);

    nAnonymizeDequantAmount = settings.value("nAnonymizeDequantAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeDequantAmount"))
        SoftSetArg("-anonymizedequantamount", settings.value("nAnonymizeDequantAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in dequant.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDequantAmount:
            return QVariant(nAnonymizeDequantAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDequantAmount:
            nAnonymizeDequantAmount = value.toInt();
            settings.setValue("nAnonymizeDequantAmount", nAnonymizeDequantAmount);
            emit anonymizeDequantAmountChanged(nAnonymizeDequantAmount);
            break;
        case CoinControlFeatures:
