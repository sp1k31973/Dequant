...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered dequant address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 DEQ.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal dequant:URI (dequant:XyZ...?message=example)
            {
...
static void NotifyzDEQReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzDEQReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzDEQReset.connect(boost::bind(NotifyzDEQReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzDEQReset.disconnect(boost::bind(NotifyzDEQReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zdeqTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
