...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zdeqTracker->HasMintTx(hash))) {
            //zDEQ stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZDEQ;
...
bool IsZDEQType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZDEQ:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZDEQ || type == TransactionRecord::MNReward) {
