...
CzDEQTracker::CzDEQTracker(std::string strWalletFile)
{
...
CzDEQTracker::~CzDEQTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzDEQTracker::Init()
{
...
bool CzDEQTracker::Archive(CMintMeta& meta)
{
...
bool CzDEQTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzDEQTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzDEQTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzDEQTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzDEQTracker::GetSerialHashes()
{
...
CAmount CzDEQTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzDEQTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzDEQTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzDEQTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzDEQTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzDEQTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzDEQTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzDEQTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzDEQTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzDEQTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzDEQTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDEQTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDEQTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzDEQTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzDEQTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzDEQTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzDEQTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzdeq from DB\n", __func__, listDeterministicDB.size());
    }
