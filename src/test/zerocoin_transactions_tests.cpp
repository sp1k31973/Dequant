...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zdeqTracker = unique_ptr<CzDEQTracker>(new CzDEQTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZDEQ_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZDEQ_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
